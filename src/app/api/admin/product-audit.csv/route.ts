...
interface ProductNode { id: string; title: string; handle: string; metafield?: { value?: string | null } | null }
interface Edge { node: ProductNode }
interface Resp { products: { edges: Edge[]; pageInfo: PageInfo } }
  ...
  const GQL = `
    query ProductsAuditCSV($first: Int!, $after: String) {
      products(first: $first, after: $after, sortKey: TITLE) {
        edges { node { id title handle metafield(namespace:"taxonomy", key:"category_slugs"){ value } } }
